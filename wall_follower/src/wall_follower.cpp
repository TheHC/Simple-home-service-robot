...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    if (move_type == STOP) {
        ...
        motor_msg.linear.x = 0.25;
        motor_msg.angular.z = -0.25;
    }
...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
    ...
    //motor_msg.angular.z=-0.3; 
    //motor_msg.linear.x=0.7;
    //motor_pub.publish(motor_msg);
}
...
int main(int argc, char** argv)
{
    ...
    motor_pub = handler.advertise<geometry_msgs::Twist>("/mobile_base/commands/velocity", 100);

    // Subscribe to the /scan topic and call the laser_callback function
    laser_sub = handler.subscribe("/scan", 1000, laser_callback);
